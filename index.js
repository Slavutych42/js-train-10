...
function customObjectFromEntries(entries) {
  ...
  const result1 = entries.map(([key, value]) => {
    if (typeof key === "number") {
      key = key.toString().replace("_");
    }
    return [key, value];
  })
  const obj = Object.fromEntries(result1);
  return obj;
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const result = Object.freeze(res)
...
function filterObjectsByKey(arr, key) {
  ...
  const fil = arr.filter((obj) => obj.hasOwnProperty(key))
...
function checkOwnership(obj1, obj2, key) {
  ...
  const result = Object.is(obj1.key, obj2.key)
  return result
}
